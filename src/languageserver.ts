...
import { debug, log, outputChannel } from "./extension.ts";
...
const MOONSHARP_RUNTIME_VERSION = "Moonsharp 2.0.0.0";
const SERVER_EXECUTABLES = {
    moonsharp: {
        win32: "moonsharpy.exe",
        linux: "moonsharpy",
        darwin: "moonsharpy",
    },
    standard: {
        win32: "lua-language-server.exe",
        linux: "lua-language-server",
        darwin: "lua-language-server",
    },
} as const;

function getRuntimeVersion(config: vscode.WorkspaceConfiguration): string {
    const version = config.get("Lua.runtime.version");
...
function registerCustomCommands(context: ExtensionContext) {
    ...
    context.subscriptions.push(
        Commands.registerCommand("lua.config", (changes) => {
            debug(
                `Received lua.config command with ${changes?.length} changes`,
            );
            const propMap: Record<string, Record<string, unknown>> = {};

            for (const data of changes) {
                debug(
                    `Processing config action: ${data.action} for key: ${data.key}`,
                );
                ...
                if (data.action === "add") {
                    ...
                    if (!Array.isArray(value)) {
                        log(
                            `Error: ${data.key} is not an Array! Cannot perform 'add' action.`,
                            "error",
                        );
    ...
    context.subscriptions.push(
        Commands.registerCommand("lua.exportDocument", async () => {
    ...
    context.subscriptions.push(
        Commands.registerCommand("lua.reloadFFIMeta", () => {
    ...
    context.subscriptions.push(
        Commands.registerCommand("lua.startServer", async () => {
    ...
    context.subscriptions.push(
        Commands.registerCommand("lua.stopServer", async () => {
...
export function reportAPIDoc(params: unknown) {
    if (!defaultClient) {
        return;
    }
...
export const createClient = (context: ExtensionContext) => {
    ...
    void defaultClient.start().catch((error) => {
        log(`LanguageClient start failed: ${error}`, "error");
    });
};
...
class LuaClient extends Disposable {
    ...
    async start() {
                ...
                workspace: {
                    configuration: (params) => {
                        ...
                        for (const item of params.items) {
                            ...
                            const config = Workspace.getConfiguration(
                                section.slice(0, sectionIndex),
                                resource,
                            );
                            results.push(
                                config.get(section.slice(sectionIndex + 1)),
                            );
                        }
        ...
        const serverOptions: ServerOptions = {
            command: command,
            transport: port
                ? { kind: TransportKind.socket, port: port }
                : undefined,
            args: commandParam,
    ...
    private async getCommand(config: vscode.WorkspaceConfiguration) {
        ...
        const executableName = getServerExecutableName(runtimeVersion, platform as NodeJS.Platform,);

        debug(
            `Platform detected: ${platform}. Checking internal server binaries.`,
        );
        debug(`Resolved runtime version: ${runtimeVersion}`);
        ...
        switch (platform) {
            ...
            case "linux":
                command = this.context.asAbsolutePath(
                    path.join("server", binDir || "bin-Linux", executableName),
                );
                await fs.promises.chmod(command, 0o777);
                break;
            case "darwin":
                command = this.context.asAbsolutePath(
                    path.join("server", binDir || "bin-macOS", executableName),
                );
                await fs.promises.chmod(command, 0o777);
                break;
            default:
    ...
    private statusBar() {
        ...
        this.disposables.push(
            client.onNotification(
                "$/status/report",
                (params: { text: string; tooltip: string }) => {
        ...
        this.disposables.push(
            this.client.onNotification(
                ...
                (params: { command: string; data: unknown }) => {
                    debug(
                        `Server requested execution of VS Code command: ${params.command}`,
                    );
    ...
    private languageConfiguration() {
        ...
        function convertStringsToRegex(config: unknown): unknown {
            if (typeof config !== "object" || config === null) return config;

            if (Array.isArray(config)) {
                return config.map((value) => convertStringsToRegex(value));
            }
            ...
            for (const key of Object.keys(record)) {
                ...
                if (typeof value === "object" && value !== null) {
                    record[key] = convertStringsToRegex(value);
                }
                if (
                    (key === "beforeText" || key === "afterText") &&
                    typeof value === "string"
                ) {
                    record[key] = new RegExp(value);
                }
            }
            return record;
        }
        ...
        this.disposables.push(
            this.client.onNotification(
                ...
                (params: { id: string; configuration: unknown }) => {
                    debug(
                        `Server updated language configuration for: ${params.id}`,
                    );
    ...
    private provideHover() {
        ...
        const provider = vscode.languages.registerHoverProvider("lua", {
            provideHover: (
                ...
                debug(
                    `Hover requested at ${position.line}:${position.character} with verbosity level: ${level}`,
                );
                ...
                return client.sendRequest<LSP.Hover | null>(
                    ...
                    const verboseResult = result as LSP.Hover & {
                        maxLevel?: number;
                    };
...
export function activate(context: ExtensionContext) {
    ...
    function didOpenTextDocument(document: TextDocument) {
        ...
        if (!defaultClient) {
            log(
                "No active client found. Creating client for the first Lua document.",
            );
            createClient(context);
        }
    }
...
export async function setConfig(changes: ConfigChange[]): Promise<boolean> {
    ...
    debug(
        `Sending lua.setConfig request to server with ${changes.length} items`,
    );
