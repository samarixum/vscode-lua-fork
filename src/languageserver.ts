...
function registerCustomCommands(context: ExtensionContext) {
    ...
    Commands.registerCommand("lua.config", (changes) => {
      ...
      for (const data of changes) {
        ...
        if (data.action === "set") {
          ...
          debug(
            `Successfully set ${data.key} to ${JSON.stringify(data.value)}`,
          );
          continue;
        }
        if (data.action === "prop") {
  ...
  context.subscriptions.push(
    Commands.registerCommand(
      "lua.showReferences",
      (uri: string, position: { line: number; character: number }, locations: ReferenceLocation[]) => {
...
class LuaClient extends Disposable {
  ...
  private async getCommand(config: vscode.WorkspaceConfiguration) {
    ...
    try {
      if (
        (await fs.promises.stat(this.context.asAbsolutePath("server/bin")))
          .isDirectory()
      ) {
        binDir = "bin";
      }
    ...
    switch (platform) {
      ...
      case "linux":
        command = this.context.asAbsolutePath(
          path.join("server", binDir || "bin-Linux", "lua-language-server"),
        );
        await fs.promises.chmod(command, 0o777);
        break;
      case "darwin":
        command = this.context.asAbsolutePath(
          path.join("server", binDir || "bin-macOS", "lua-language-server"),
        );
        await fs.promises.chmod(command, 0o777);
        break;
      default:
